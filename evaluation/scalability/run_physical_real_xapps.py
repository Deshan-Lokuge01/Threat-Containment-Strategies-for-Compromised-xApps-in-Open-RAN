...
RESULT_ROOT = Path("experiments/scalability/results/exp3-realxapps-20260807T175932Z")
JSONL = RESULT_ROOT / "real_concurrent_trials.jsonl"
SUMMARY = RESULT_ROOT / "real_concurrent_summary.json"
MANIFEST = RESULT_ROOT / "real_concurrent_manifest.json"
...
def run_cell(k: int, repetition: int) -> dict:
    ...
    baseline = {name: reachable(snapshots[name]["ip"]) for name in targets}
    ...
    def observe_probe(name: str) -> tuple[str, dict]:
        ...
        return name, {
            "unreachable": not reachable(snapshots[name]["ip"]),
    ...
    for name in targets:
        try:
            ...
            restored[name] = {
                "fresh_pod": fresh,
                "service_reachable": reachable(fresh["ip"]),
...
def main() -> int:
    ...
    MANIFEST.write_text(json.dumps({
        ...
        "observation": "200 ms polling plus independent pod-IP:4560 TCP and iptables checks",
        "active_probe_execution": "one concurrent TCP connect attempt per target pod IP on RMR port 4560",
        ...
        "claim_boundary": (
            "The four real O-RAN SC xApps (hw-go, hw-python, kpimon-go, trafficxapp) "
            "exercise the real Kubernetes/Falco/evaluator/responder containment path. "
            "Only kpimon-go is E2 control-loop active; the others are real deployed "
            "xApps without live E2 KPI traffic."
        ),
        "created_utc": utc_now(),
    }, indent=2) + "\n")
