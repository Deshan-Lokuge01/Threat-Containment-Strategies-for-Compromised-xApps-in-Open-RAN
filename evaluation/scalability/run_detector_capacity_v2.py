...
def worker(args: argparse.Namespace) -> int:
    ...
    # Deterministic circular offsets (Safeguard: reproducible + decorrelated).
    # Offset for context i is fixed by max_contexts so context i uses the same
    # trace phase at every N, which is what the decision-equivalence check below
    # relies on. total_ticks = warmup + measured; the warmup fills MEWMA/history
    # state and is EXCLUDED from the steady-state latency distribution.
    total_ticks = args.warmup_ticks + args.ticks
    usable = len(trace) - total_ticks
    stride = max(1, usable // max(1, args.max_contexts - 1))
    offsets = [(i * stride) % (usable + 1) for i in range(args.workloads)]
    counts = [{"updates": 0, "ready": 0, "normal": 0, "suspicious": 0} for _ in scorers]
    ctx_hash = [hashlib.sha256() for _ in scorers]
    gc.collect()
    start_rss = rss_bytes()
    ...
    for tick in range(total_ticks):
        measured = tick >= args.warmup_ticks
        rng.shuffle(order)
        ...
        for index in order:
            ...
            ctx_hash[index].update((result.public_state + "|").encode())
            item = counts[index]
            item["updates"] += 1
            if result.ready:
                ready += 1
    ...
    end_rss = rss_bytes()
    # Decision-equivalence (Safeguard C): rerun each context in isolation and
    # confirm its full NORMAL/SUSPICIOUS/COMPROMISED sequence is identical to the
    # sequence it produced alongside the other N-1 contexts. This is NOT timed.
    decision_mismatches = 0
    for i in range(args.workloads):
        solo = StreamingScorer(spec=spec)
        solo_hash = hashlib.sha256()
        for tick in range(total_ticks):
            sample = dict(trace[offsets[i] + tick])
            sample["elapsed_s"] = float(tick + 1)
            r = solo.update(sample)
            solo_hash.update((r.public_state + "|").encode())
        if solo_hash.hexdigest() != ctx_hash[i].hexdigest():
            decision_mismatches += 1
    decision_equiv_ok = decision_mismatches == 0
    measured_cycles = [row for row in cycles if row["measured"]]
    latencies = [float(row["cycle_latency_ms"]) for row in measured_cycles]
    accounting = []
    ...
    total_ticks = args.warmup_ticks + args.ticks
    expected = args.workloads * total_ticks
    observed = sum(row["updates"] for row in accounting)
    if observed != expected or any(row["updates"] != total_ticks for row in accounting):
        raise RuntimeError(f"accounting mismatch: expected={expected}, observed={observed}")
    result = {
        "schema": "zt-xguard-detector-capacity-worker-v2",
        ...
        "measured_ticks": len(measured_cycles),
        "ticks": args.ticks,
        "max_contexts": args.max_contexts,
        "decision_equiv_ok": decision_equiv_ok,
        ...
        "cycle_latency_p50_ms": statistics.median(latencies),
        ...
        "deadline_misses": sum(int(row["deadline_missed"]) for row in measured_cycles),
        "deadline_miss_rate": statistics.fmean(int(row["deadline_missed"]) for row in measured_cycles),
        "cycles": cycles,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-contexts", type=int, default=80)
