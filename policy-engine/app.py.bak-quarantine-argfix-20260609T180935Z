...
# cache, and Falco/runtime events update it immediately.
...
# ------------------------------------------------------------
# ZT_XGUARD_FALLBACK_CONTAINMENT_MARKER
# Fallback containment for CNIs that do not enforce NetworkPolicy
# ------------------------------------------------------------
...
def service_endpoints_summary(service_name: str, namespace: str = XAPP_NAMESPACE) -> Dict[str, Any]:
    """Summarize legacy Endpoints object for service containment verification."""
    try:
        ...
        for subset in ep.subsets or []:
            for addr in subset.addresses or []:
                addresses.append(getattr(addr, "ip", None))
...
def apply_service_isolation(
    ...
    for svc in services:
        ...
        # Give endpoints controller a tiny moment to update.
        time.sleep(0.2)
...
def restore_service_isolation(
    ...
    for svc in services:
        ...
        # Build a merge patch that removes any stale selector keys not present
        # in the original selector. This is the key fix.
        selector_patch = {}

        for key in current_selector.keys():
            if key not in original_selector:
                selector_patch[key] = None

        for key, value in original_selector.items():
            selector_patch[key] = value

        patch = {
            "metadata": {
                "annotations": {
                    ORIGINAL_SELECTOR_ANNOTATION: None,
                    SERVICE_ISOLATED_ANNOTATION: None,
                    SERVICE_ISOLATED_INCIDENT_ANNOTATION: None,
                    "zt-xguard.io/service-isolated-time": None,
                    "zt-xguard.io/service-isolated-reason": None,
                    "zt-xguard.io/service-isolated-selector-token": None,
                    "zt-xguard.io/xapp": None,
                }
            },
            "spec": {
                "selector": selector_patch
            }
        }
...
# Keep the original NetworkPolicy quarantine implementation but wrap it with
# fallback containment. Existing code calls apply_quarantine(), so redefining it
# here changes behavior without needing to rewrite the old event path.
_ztx_original_apply_quarantine = apply_quarantine
...
def apply_quarantine(pod_name: str, namespace: str, reason: str, incident_id: str) -> Dict[str, Any]:
    ...
    try:
        netpol_result = _ztx_original_apply_quarantine(pod_name, namespace, reason, incident_id)
    ...
    if mode in ["service", "service_then_scale", "auto"]:
        try:
    ...
    # In this prototype environment, NetworkPolicy exists but is not enforced by the CNI.
    # Therefore, service-mode containment is only successful if Service isolation succeeds.
...
def verify_xapp_containment(xapp: str, namespace: str = XAPP_NAMESPACE) -> Dict[str, Any]:
    services = list_services_for_xapp(xapp, namespace)
    service_results = []
    ...
    for svc in services:
        ...
        service_results.append({
            ...
            "annotations": svc.metadata.annotations or {},
            "endpoint_summary": ep,
            "service_isolated": ep.get("has_endpoints") is False,
        })
    ...
    for pod in get_pods_for_xapp(xapp, namespace):
        ...
        pod_results.append({
            ...
            "labels": {
                ...
                if k.startswith("zt-xguard.io/") or k in ["security-status", "app"]
            },
            "quarantine_label": labels.get("zt-xguard.io/quarantine") == "true",
        })

    service_isolated = any(item.get("service_isolated") is True for item in service_results)
    verification_unknown = any(
        item.get("endpoint_summary", {}).get("has_endpoints") is None
        for item in service_results
    )
    pod_labelled = any(item.get("quarantine_label") for item in pod_results)
    ...
    return {
        ...
        "contained": service_isolated and not verification_unknown,
        "quarantine_marked": pod_labelled,
        "verification_unknown": verification_unknown,
        "containment_note": "contained=true only when service isolation is verified; pod quarantine label alone is not treated as effective containment in this CNI-limited testbed",
...
def _ztx_original_csm_process_falco_event(alert: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
    ...
    if result.get("state") == "COMPROMISED" and xapp in XAPP_LIST and pod_name != "unknown":
        if AUTO_QUARANTINE:
            q_started = time.time()
...
def csm_audit() -> Any:
    ...
    report = build_report(persist=persist, reason="csm_audit")
...
def csm_quarantine() -> Any:
    ...
    incident_id = payload.get("incident_id") or f"{utc_id()}-manual-quarantine-{safe_name(xapp or pod_name or 'unknown')}"
    ...
    csm_update_from_result(result, source="manual_quarantine")
...
def csm_events_falco() -> Any:
    ...
    body, status = ztx_normalize_result(csm_process_falco_event(alert))
...
def csm_containment_apply() -> Any:
    ...
    incident_id = payload.get("incident_id") or f"{utc_id()}-manual-containment-{safe_name(xapp or pod_name or 'unknown')}"

    if not pod_name and xapp:
        pod = get_primary_pod_for_xapp(xapp, namespace)
        pod_name = pod.metadata.name if pod else None
    ...
    verify = verify_xapp_containment(result.get("xapp") or xapp or "unknown", namespace)
...
def csm_containment_restore() -> Any:
    ...
    for pod in get_pods_for_xapp(xapp, namespace):
        ...
        try:
            CORE.patch_namespaced_pod(name=pod_name, namespace=namespace, body=patch)
            pod_results.append({"pod": pod_name, "labels_removed": True})
        except Exception as exc:
            pod_results.append({"pod": pod_name, "labels_removed": False, "error": str(exc)})

    verify = verify_xapp_containment(xapp, namespace)

    return jsonify({
        "restored": service_restore.get("restored", False) or scale_restore.get("restored", False) or any(p.get("labels_removed") for p in pod_results),
        "xapp": xapp,
        "namespace": namespace,
...
def ztx_apply_scenario_containment_if_required(scenario, xapp, namespace, event_result):
    ...
    try:
        containment = apply_quarantine(xapp, namespace, incident_id, reason)
        return {
            "attempted": True,
...
def ztx_v4_signal_from_event(event):
    ...
    if event.get("ztx_signal"):
        return str(event.get("ztx_signal"))

    if fields.get("ztx.signal"):
        return str(fields.get("ztx.signal"))
...
def ztx_v4_apply_quarantine_compat(xapp, namespace, incident_id, reason):
    pod_name = None
    try:
        pod = get_primary_pod_for_xapp(xapp, namespace)
        if pod:
    ...
    try:
        return apply_quarantine(xapp, namespace, incident_id, reason)
    except Exception as exc:
        attempts.append(f"xapp_signature_failed={exc}")

    try:
        return apply_quarantine(xapp=xapp, namespace=namespace, incident_id=incident_id, reason=reason)
    except Exception as exc:
        attempts.append(f"kw_xapp_signature_failed={exc}")

    if pod_name:
        try:
            return apply_quarantine(pod_name, namespace, reason, incident_id)
        except Exception as exc:
            attempts.append(f"pod_signature_failed={exc}")

    return {
        "applied": False,
        "error": "all_apply_quarantine_signatures_failed",
        "attempts": attempts,
    }
...
def ztx_v4_process_signal(xapp, signal, evidence=None, namespace=None, source="intent_signal"):
    ...
    if decision["action"] == "FORENSIC_QUARANTINE":
        ...
        q_started = time.time()
        containment = ztx_v4_apply_quarantine_compat(xapp, namespace, incident_id, reason)
        quarantine_ms = round((time.time() - q_started) * 1000, 3)
