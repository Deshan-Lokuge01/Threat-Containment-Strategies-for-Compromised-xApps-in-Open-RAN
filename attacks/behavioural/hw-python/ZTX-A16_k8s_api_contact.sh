...
ztx_trigger "$POD" "hw-python" python3 -c 'import ssl,urllib.request; ctx=ssl.create_default_context(); ctx.check_hostname=False; ctx.verify_mode=ssl.CERT_NONE; urllib.request.urlopen('\''https://kubernetes.default.svc.cluster.local'\'', timeout=3, context=ctx)'
