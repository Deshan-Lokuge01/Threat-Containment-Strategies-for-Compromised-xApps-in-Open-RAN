...
def ztx_apply_scenario_containment_if_required(scenario, xapp, namespace, event_result):
    ...
    try:
        ...
        containment = apply_quarantine(pod_name, namespace, reason, incident_id)
        return {
            "attempted": True,
...
def ztx_v4_apply_quarantine_compat(xapp, namespace, incident_id, reason):
    ...
    if pod_name:
        try:
            result = apply_quarantine(pod_name, namespace, reason, incident_id)
    ...
    try:
        result = apply_quarantine(xapp, namespace, reason, incident_id)
