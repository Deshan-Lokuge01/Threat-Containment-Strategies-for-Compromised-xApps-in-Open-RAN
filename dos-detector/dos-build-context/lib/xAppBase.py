...
# Safe imports for E2SM modules (handles both relative and top-level package resolution)
try:
    from .e2sm_ccc_module import e2sm_ccc_module
except (ImportError, ModuleNotFoundError):
    try:
        from lib.e2sm_ccc_module import e2sm_ccc_module
    except (ImportError, ModuleNotFoundError):
        e2sm_ccc_module = None

try:
    from .e2sm_kpm_module import e2sm_types, e2sm_kpm_module
except (ImportError, ModuleNotFoundError):
    try:
        from lib.e2sm_kpm_module import e2sm_types, e2sm_kpm_module
    except (ImportError, ModuleNotFoundError):
        e2sm_types = None
        e2sm_kpm_module = None
...
class SubscriptionWrapper(object):
    def __init__(self):
        ...
        self.e2sm_type = e2sm_types.E2SM_UNKNOWN if e2sm_types else None
...
class xAppBase(object):
    def __init__(self, config=None, http_server_port=8090, rmr_port=4560, rmr_flags=0x00):
        ...
        self.SUB_MGR_URI = "http://service-ricplt-submgr-http.ricplt.svc.cluster.local:8080/ric/v1"
        ...
        # Safely instantiate optional modules
        self.e2sm_ccc = e2sm_ccc_module(self) if e2sm_ccc_module else None
        self.e2sm_kpm = e2sm_kpm_module(self) if e2sm_kpm_module else None
        self.e2sm_rc = e2sm_rc_module(self) if e2sm_rc_module else None
        ...
        # --- STEP 1: START HTTP HEALTH SERVER FIRST ---
        # Moving this above the RMR loop prevents K8s probes from failing with 'connection refused'
        ...
        if self.subscriber.ResponseHandler(self._subscription_response_callback, self.httpServer) is not True:
            print("Error when trying to set the subscription response callback")
        
        self.httpServer.start()
        print(f"HTTP Server started on {self.MY_HTTP_SERVER_ADDRESS}:{self.MY_HTTP_SERVER_PORT}")
        ...
        # --- STEP 2: INITIALIZE RMR CLIENT SECOND ---
        ...
        print("Waiting for RMR routing table initialization...")
    ...
    def subscribe(self, e2_node_id, ran_function_id, event_trigger_def, action_def, indication_callback, e2sm_type=None):
        if e2sm_type is None and e2sm_types:
            e2sm_type = e2sm_types.E2SM_UNKNOWN
                ...
                if (summary['message type'] == 12050):
                    ...
                    try:
                        ...
                        if callback_func is not None:
                            if (e2sm_types and subscriptionObj.e2sm_type == e2sm_types.E2SM_KPM and self.e2sm_kpm):
